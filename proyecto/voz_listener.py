...
class VozListener:
    def __init__(self, modelo_path="/home/tacho/Documents/qubit/proyecto/stt/vosk-model-small-es-0.42", sample_rate=16000):
    ...
    def identificar_hablante(self, nueva_voz):
        ...
        if confianza > 0.75:
            nombre = nombres[idx]
            nombre = re.sub(r'\d+', '', nombre)  # Quita números
...
# Atajo práctico
def escuchar_voz():
    listener = VozListener()
    texto, persona, confianza = listener.escuchar()
    return texto, persona, confianza

if __name__ == "__main__":
    texto, persona, conf = escuchar_voz()
    print(f"Habló: {persona} (confianza {conf})")
    print(f"Dijo: {texto}")
